...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Only proceed if dropdowns exist (for pages that have them)
    if (primaryDropdown && subCategoryDropdown) {
...
//CHARMI ADDED CODE -16/7/25
// APPOINTMENT DATE CALCULATION - FIXED VERSION
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (dateEl) {
        ...
        // If visaOffsets is defined from PHP and a visa type is selected
        const selectedVisaType = document.getElementById('primary-category')?.value;

        if (typeof visaOffsets !== "undefined" && selectedVisaType && visaOffsets[selectedVisaType.toLowerCase()]) {
            offsetDays = visaOffsets[selectedVisaType.toLowerCase()];
        }
